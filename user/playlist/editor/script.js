...
function addAudio(button) {
   ...
   const randId = rand.dataset.playlist

   if (button.querySelector('.rotate45')){
      button.querySelector('img').classList.remove('rotate45')
      var xhr = new XMLHttpRequest()
       let url = new URL('http://t96800rh.beget.tech/user/playlist/editor/remove.php')
      ...
      var xhr = new XMLHttpRequest()
       let url = new URL('http://t96800rh.beget.tech/user/playlist/editor/add.php')
       url.searchParams.set('hash', randId)
       xhr.open('GET', url, true)
       xhr.send()
       url.searchParams.set('id', songId)
